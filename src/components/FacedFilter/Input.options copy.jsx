...
export const SelectBox = ({
  ...
  console.log(defaultValue, "final");
  ...
  useEffect(() => {
    // Ensure defaultValue is propagated to internal state and parent's onChange
    let finalValue = null;
    ...
    if (typeof defaultValue === "object") {
      ...
      if (isMulti) {
        const valueArray = Array.isArray(defaultValue)
          ? defaultValue.map((v) => String(v).toLowerCase())
          : String(defaultValue)
              .split(",")
              .map((v) => v.trim().toLowerCase());

        finalValue = options.filter((option) =>
          valueArray.includes(String(option[valueField]).toLowerCase()),
        );
      } else {
        finalValue =
          options.find(
            (option) =>
              String(option[valueField]).toLowerCase() ===
              String(defaultValue).toLowerCase(),
          ) || null;
      }
    }

    // If we computed a finalValue, apply it via handleChange so internal state and
    // parent's onChange handler are invoked consistently.
    if (finalValue !== null) {
      handleChange(finalValue);
    }
  }, [defaultValue, options, isMulti, valueField]);
