...
export const createProduct = asyncErrorHandler(async (req) => {
  let { name, mrp, specialPrice, gst, cost, primaryUnit } = req.body;
  ...
  userActivity({
    ...
    description: `Product "${name} - ${uniqueId}" created by {userName}`,
  });
...
export const updateProduct = asyncErrorHandler(async (req) => {
  let { id, name, mrp, specialPrice, gst, cost, primaryUnit } = req.body;
  ...
  let updated = await models.Product.findOneAndUpdate(
    { _id: id, status: { $ne: 1 } },
    data,
  );
  ...
  userActivity({
    ...
    description: `Product "${name} - ${updated?.uniqueId}" updated by {userName}`,
  });
...
export const getProduct = asyncErrorHandler(async (req) => {
  ...
  let data = await models.Product.find(
    condition,
    "name mrp specialPrice gst cost primaryUnit status date",
  )
    .sort({ _id: -1 })
    .skip(skip)
    .limit(limit)
...
export const getProductDetails = asyncErrorHandler(async (req) => {
  ...
  let data = await models.Product.findOne(
    condition,
    "name mrp specialPrice gst cost primaryUnit status date",
  );
...
export const deleteProduct = asyncErrorHandler(async (req) => {
  ...
  userActivity({
    ...
    description: `Product "${updated?.name} - ${updated?.uniqueId}" deleted by {userName}`,
  });
...
export const statusChange = asyncErrorHandler(async (req) => {
  ...
  userActivity({
    req,
    action: ACTIVITY_ACTIONS.INVENTORY.ITEM_STATUS,
    description: `Product "${updated?.name}" status changed by {userName}`,
  });
