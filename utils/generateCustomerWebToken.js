...
const generateWebTokens = async (data) => {
  try {
    ...
    const refreshToken = jwt.sign(
     ...
     await models.CustomerWebToken.findOneAndDelete({customerId: data._id,});

    await new models.CustomerWebToken({ customerId: data._id, token: refreshToken }).save();
